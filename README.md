...
Qt 5.15 приложение для запроса данных о телах системы через EDSM/Spansh/EDAstro API и отрисовки схемы связей (планеты, спутники, родительские тела).
...
- **Только EDAstro (по умолчанию)** — запрос к `https://edastro.com/api/starsystem?q={starsystem}`; используется как основной источник и подходит для формата `eadstro_example.json`.
...
- **Только Spansh** — перед запросом к Spansh индекс системы определяется через EDSM (`/api-v1/system`) с параметром `showId=1`, чтобы гарантированно получить идентификатор системы.
