...
Qt 5.15 приложение для запроса данных о телах системы через EDAstro API и отрисовки схемы связей (планеты, спутники, родительские тела).

## Возможности
- Запрос по названию системы (например, `Sol`).
- Загрузка списка тел из `https://edastro.com/api/starsystem?q={starsystem}` (структура как в `eadstro_example.json`).
...
## Источник данных
В текущей версии для стабильной работы включён только **EDAstro**:
- запрос идёт к `https://edastro.com/api/starsystem?q={starsystem}`;
- парсинг использует структуру массивов `stars`, `planets`, `moons`, `barycentres/barycenters`;
- альтернативные источники (EDSM/Spansh) отключены в UI.
